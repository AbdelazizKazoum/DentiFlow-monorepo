  ...
  implements StaffRepository
{
  ...
  private get base(): string {
    return `/api/v1/clinics/${this.clinicId}/staff`;
  }
  ...
  async findByClinicId(clinicId: string): Promise<Staff[]> {
    const response = await this.execute(() =>
      axiosClient.get<StaffListDTO>(`/api/v1/clinics/${clinicId}/staff`),
    );
    return response.data.staffMembers.map(toDomain);
  }
  ...
  async findByUserId(userId: string): Promise<Staff | null> {
    try {
      const response = await this.execute(() =>
        axiosClient.get<StaffDTO>(`${this.base}/${userId}`),
      );
  ...
  async update(id: string, input: UpdateStaffInput): Promise<Staff> {
    const dto = toUpdateDTO(input);
    const response = await this.execute(() =>
      axiosClient.put<StaffDTO>(`${this.base}/${id}`, dto),
    );
    return toDomain(response.data);
  }
