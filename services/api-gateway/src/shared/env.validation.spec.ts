...
describe("gatewaySchema", () => {
  ...
  it("resolves with defaults when only JWT_SECRET is provided", () => {
    ...
    expect(value.PATIENT_SERVICE_URL).toBe("http://patient-service:3004");
    expect(value.PATIENT_SERVICE_GRPC_URL).toBe("patient-service:5003");
  });
  ...
  it("accepts and coerces a full valid configuration", () => {
    const {value, error} = gatewaySchema.validate({
      ...
      AUTH_SERVICE_GRPC_URL: "auth:5001",
      CLINIC_SERVICE_GRPC_URL: "clinic:5002",
      PATIENT_SERVICE_GRPC_URL: "patient:5003",
    });
    expect(error).toBeUndefined();
